import { useState, useMemo, useCallback, useEffect, useRef } from 'react';
import { APIProvider, Map, AdvancedMarker, InfoWindow, Pin } from '@vis.gl/react-google-maps';

// ----- Helper Function: Haversine Distance ----- (No changes needed)
function calculateDistance(pos1, pos2) {
    if (!pos1 || !pos2) return Infinity;
    const R = 6371; // km
    const dLat = (pos2.lat - pos1.lat) * Math.PI / 180;
    const dLon = (pos2.lng - pos1.lng) * Math.PI / 180;
    const a = 0.5 - Math.cos(dLat)/2 + Math.cos(pos1.lat * Math.PI / 180) * Math.cos(pos2.lat * Math.PI / 180) * (1 - Math.cos(dLon))/2;
    const distanceKm = R * 2 * Math.asin(Math.sqrt(a));
    return distanceKm * 0.621371; // miles
}

// ----- Helper Component: Modal ----- (No changes needed)
const EventFormModal = ({ isOpen, onClose, onSubmit, position, eventDetails, setEventDetails, isSubmitting }) => {
     if (!isOpen) return null;
    const handleChange = (e) => {
        const { name, value } = e.target;
        setEventDetails(prev => ({ ...prev, [name]: value }));
    };
    const handleSubmit = (e) => {
        e.preventDefault();
        onSubmit(eventDetails);
    };
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 shadow-xl w-full max-w-md text-black">
                <h2 className="text-2xl font-bold mb-4 text-gray-800">Add New Event</h2>
                {position && <p className="text-sm text-gray-600 mb-4">
                    Location: Lat: {position.lat.toFixed(4)}, Lng: {position.lng.toFixed(4)}
                </p>}
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1">Title</label>
                        <input type="text" id="title" name="title" value={eventDetails.title} onChange={handleChange} required disabled={isSubmitting} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100"/>
                    </div>
                    <div className="mb-4">
                        <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                        <textarea id="description" name="description" value={eventDetails.description} onChange={handleChange} rows="3" required disabled={isSubmitting} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100"></textarea>
                    </div>
                    <div className="mb-4">
                        <label htmlFor="category" className="block text-sm font-medium text-gray-700 mb-1">Category</label>
                        <select id="category" name="category" value={eventDetails.category} onChange={handleChange} required disabled={isSubmitting} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 bg-white disabled:bg-gray-100">
                            {/* Populate options dynamically based on config if needed, or keep static */}
                            <option value="event">Event</option>
                            <option value="landmark">Landmark</option>
                            <option value="Generic">Generic</option> {/* Example if backend uses this */}
                            {/* Add other categories if needed based on backend */}
                        </select>
                    </div>
                    <div className="mb-4">
                        <label htmlFor="startTime" className="block text-sm font-medium text-gray-700 mb-1">Start Time</label>
                        <input type="datetime-local" id="startTime" name="startTime" value={eventDetails.startTime} onChange={handleChange} required disabled={isSubmitting} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100"/>
                    </div>
                    <div className="mb-4">
                        <label htmlFor="endTime" className="block text-sm font-medium text-gray-700 mb-1">End Time</label>
                        <input type="datetime-local" id="endTime" name="endTime" value={eventDetails.endTime} onChange={handleChange} required disabled={isSubmitting} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100"/>
                    </div>
                    <div className="flex justify-end space-x-3 mt-6">
                        <button type="button" onClick={onClose} disabled={isSubmitting} className="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 transition-colors disabled:opacity-50">Cancel</button>
                        <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-wait">{isSubmitting ? 'Adding...' : 'Add Event'}</button>
                    </div>
                </form>
            </div>
        </div>
    );
};

// ----- Helper Component: User Location Marker ----- (No changes needed)
const UserLocationMarker = () => (
    <div style={{ width: '16px', height: '16px', borderRadius: '50%', backgroundColor: 'rgba(66, 133, 244, 0.7)', border: '2px solid white', boxShadow: '0 0 5px rgba(0, 0, 0, 0.3)', transform: 'translate(-50%, -50%)' }} />
);

// ----- Main App Component -----
function App() {
    // ----- STATE -----
    const [appConfig, setAppConfig] = useState(null); // Holds config from /api/config
    const [isLoadingConfig, setIsLoadingConfig] = useState(true);
    const [configError, setConfigError] = useState(null);
    // --- Event State ---
    const [events, setEvents] = useState([]);
    const [selectedEvent, setSelectedEvent] = useState(null);
    const [temporaryMarkerPosition, setTemporaryMarkerPosition] = useState(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isSubmittingEvent, setIsSubmittingEvent] = useState(false);
    const [newEventDetails, setNewEventDetails] = useState({ title: '', description: '', category: 'event', startTime: '', endTime: '' });
    const [isLoadingEvents, setIsLoadingEvents] = useState(false);
    const [error, setError] = useState(null); // For general/event-related errors
    const [proximityError, setProximityError] = useState(null);
    // --- Map & Location State ---
    const defaultCenter = useMemo(() => ({ lat: 39.8283, lng: -98.5795 }), []);
    const [mapCenter, setMapCenter] = useState(defaultCenter);
    const [initialZoom, setInitialZoom] = useState(4);
    const [currentUserPosition, setCurrentUserPosition] = useState(null);
    const [isLoadingLocation, setIsLoadingLocation] = useState(true);
    const watchIdRef = useRef(null);

    // ----- CONSTANTS -----
    const API_BASE_URL = "https://collabamap.8ken.biz/api"; // Adjust if your proxy/backend differs
    const EVENT_FETCH_RADIUS_MILES = 25; // Radius for fetching existing events (can be adjusted)
    // EVENT_CREATION_RADIUS_MILES is now fetched from config

    // ----- EFFECTS -----

    // Effect 0: Fetch Configuration on Mount
    useEffect(() => {
        const fetchConfig = async () => {
            setIsLoadingConfig(true);
            setConfigError(null);
            try {
                const response = await fetch(`${API_BASE_URL}/config`);
                if (!response.ok) {
                    throw new Error(`Failed to fetch config: ${response.status} ${response.statusText}`);
                }
                const configData = await response.json();
                if (!configData.maps_api_key) {
                    console.log("mapsapi key didnt work");
                    throw new Error("Maps API key missing from config response.");
                }
                console.log("App Config Loaded:", configData);
                setAppConfig(configData);
            } catch (err) {
                console.error("Error fetching configuration:", err);
                setConfigError(`Config Load Failed: ${err.message}. Map functionality may be limited.`);
                setAppConfig(null); // Ensure config is null on error
            } finally {
                setIsLoadingConfig(false);
            }
        };
        fetchConfig();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []); // Empty dependency array ensures this runs only once on mount


     // Effect 1: Get Initial Location & Start Watching Position
    useEffect(() => {
        // No changes needed in the geolocation logic itself
        if (!navigator.geolocation) {
            setError("Geolocation is not supported."); setIsLoadingLocation(false); return;
        }
        let isMounted = true;
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                if (!isMounted) return;
                const loc = { lat: pos.coords.latitude, lng: pos.coords.longitude };
                setMapCenter(loc); setInitialZoom(11); setCurrentUserPosition(loc); setError(null); setIsLoadingLocation(false);
                if (!watchIdRef.current) {
                    watchIdRef.current = navigator.geolocation.watchPosition(
                        (updPos) => { if (isMounted) setCurrentUserPosition({ lat: updPos.coords.latitude, lng: updPos.coords.longitude }); },
                        (err) => { console.error("Watch err:", err); if (isMounted) setError(prev => `${prev || ''} | Watch Error: ${err.message}`); },
                        { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }
                    );
                }
            },
            (err) => { if (!isMounted) return; setError(`Location Error: ${err.message}. Using default.`); setIsLoadingLocation(false); },
            { enableHighAccuracy: true, timeout: 10000, maximumAge: 60000 }
        );
        return () => { isMounted = false; if (watchIdRef.current) navigator.geolocation.clearWatch(watchIdRef.current); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [defaultCenter]);

     // Effect 2: Fetch initial events (Adjusted dependency)
    useEffect(() => {
        // Wait for location AND config to be ready before fetching events
        if (isLoadingLocation || isLoadingConfig || !appConfig || mapCenter === defaultCenter) return;

        const fetchEvents = async () => {
            setIsLoadingEvents(true); setError(null); setProximityError(null);
            // Assuming GET /events still takes lat/lng/radius
            const url = `${API_BASE_URL}/events?lat=${mapCenter.lat}&lng=${mapCenter.lng}`;
            try {
                const response = await fetch(url);
                if (!response.ok) throw new Error(`Fetch failed: ${response.status}`);
                const fetchedEventsFromServer = await response.json();

                // --- MAP GET RESPONSE TO FRONTEND STATE STRUCTURE ---
                // This mapping is crucial if the GET /events response structure matches the POST response structure
                const formattedEvents = fetchedEventsFromServer.map(event => ({
                    id: event._id,
                    title: event.title,
                    description: event.description,
                    category: event.category,
                    position: {
                        lat: event.location?.coordinates[1], // Added optional chaining for safety
                        lng: event.location?.coordinates[0]
                    },
                    startTime: event.started_at,
                    endTime: event.ended_at
                })).filter(event => event.position && event.position.lat != null && event.position.lng != null); // Filter out events with invalid location data

                console.log("Formatted Events from GET:", formattedEvents);
                setEvents(formattedEvents);

            } catch (err) { setError(`Event Load failed: ${err.message}`); setEvents([]); }
            finally { setIsLoadingEvents(false); }
        };
        fetchEvents();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [mapCenter, isLoadingLocation, isLoadingConfig, appConfig]); // Depend on config loading state too


    // ----- EVENT HANDLERS -----

    // MODIFIED: Handle map click - Uses config radius
    const handleMapClick = useCallback((event) => {
        setSelectedEvent(null); // Deselect any event when clicking map bg
        if (!event.detail.latLng) return;

        // Need config and current location to check proximity
        if (!appConfig || !currentUserPosition) {
            setTemporaryMarkerPosition(null);
            setProximityError(appConfig ? "Current location unavailable." : "Configuration not loaded.");
            return;
        }

        const clickedPosition = { lat: event.detail.latLng.lat, lng: event.detail.latLng.lng };
        const distance = calculateDistance(currentUserPosition, clickedPosition);
        const creationRadius = appConfig.event_creation_radius; // Get radius from config

        if (distance <= creationRadius) {
            setTemporaryMarkerPosition(clickedPosition);
            setProximityError(null); // Clear proximity error if within range
        } else {
            setTemporaryMarkerPosition(null); // Clear marker if too far
            setProximityError(`Must be within ${creationRadius.toFixed(1)} mile(s) to add event.`);
        }
    }, [currentUserPosition, appConfig]); // Depends on appConfig now

    // Handle add event click (No changes needed)
    const handleAddEventClick = () => {
        if (temporaryMarkerPosition) {
            setNewEventDetails({ title: '', description: '', category: 'event', startTime: '', endTime: '' });
            setIsModalOpen(true);
            setProximityError(null); // Clear proximity error when opening modal
        }
    };

    // Handle close modal (No changes needed)
    const handleCloseModal = () => {
        setIsModalOpen(false);
        setTemporaryMarkerPosition(null); // Clear temp marker when closing modal
        setIsSubmittingEvent(false);
    };

    // Handle submitting the new event form (Logic remains the same, using fetch)
    const handleEventSubmit = async (details) => {
        if (!temporaryMarkerPosition) {
            setError("Cannot submit event without a location.");
            return;
        }

        setIsSubmittingEvent(true);
        setProximityError(null);
        setError(null);

        const url = `${API_BASE_URL}/events?lat=${temporaryMarkerPosition.lat}&lng=${temporaryMarkerPosition.lng}`;

        let requestBody;
        try {
            const startTimeISO = details.startTime ? new Date(details.startTime).toISOString() : null;
            const endTimeISO = details.endTime ? new Date(details.endTime).toISOString() : null;

            if (!startTimeISO || !endTimeISO) {
                throw new Error("Invalid start or end time provided.");
            }
            if (new Date(endTimeISO) <= new Date(startTimeISO)) {
                throw new Error("End time must be after start time.");
            }

            requestBody = {
                title: details.title,
                description: details.description,
                category: details.category,
                start_time: startTimeISO, // Use backend's expected field name
                end_time: endTimeISO,     // Use backend's expected field name
            };
        } catch (prepError) {
             console.error("Error preparing event data:", prepError);
             setError(`Data Error: ${prepError.message}`);
             setIsSubmittingEvent(false);
             return;
        }

        console.log("Submitting to URL:", url);
        console.log("Submitting Body:", requestBody);

        try {
            const response = await fetch(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(requestBody),
            });

            if (response.ok) {
                const createdEventFromServer = await response.json();
                console.log("Successfully added event (Server Response):", createdEventFromServer);

                // Map Server Response to Frontend State Structure
                const newEventForState = {
                    id: createdEventFromServer._id,
                    title: createdEventFromServer.title,
                    description: createdEventFromServer.description,
                    category: createdEventFromServer.category,
                    position: {
                        lat: createdEventFromServer.location?.coordinates[1], // Added optional chaining
                        lng: createdEventFromServer.location?.coordinates[0],
                    },
                    startTime: createdEventFromServer.started_at,
                    endTime: createdEventFromServer.ended_at,
                };

                // Ensure position data is valid before adding
                if (newEventForState.position && newEventForState.position.lat != null && newEventForState.position.lng != null) {
                   setEvents(prevEvents => [...prevEvents, newEventForState]);
                   handleCloseModal(); // Close modal on success
                } else {
                    console.error("Received created event with invalid location data:", createdEventFromServer);
                    setError("Event created, but location data is missing or invalid in the response.");
                    setIsSubmittingEvent(false); // Keep modal open if data is bad
                }

            } else if (response.status === 422) {
                const errorData = await response.json();
                console.error("Validation Error (422):", errorData);
                const errorMessages = errorData.detail?.map(err => `${err.loc?.join('.')} : ${err.msg}`).join('; ') || 'Validation failed.';
                setError(`Validation Error: ${errorMessages}`);
                setIsSubmittingEvent(false);
            } else {
                const errorText = await response.text();
                throw new Error(`Failed to add event: ${response.status} ${response.statusText}. ${errorText}`);
            }

        } catch (err) {
            console.error("Error adding event:", err);
            setError(`Failed to add event: ${err.message}`);
            setIsSubmittingEvent(false);
        }
    };

    // Handle selecting event (No changes needed)
    const handleSelectEvent = (event) => {
        setSelectedEvent(event);
        setTemporaryMarkerPosition(null);
        setProximityError(null);
    };

    // ----- RENDER -----
    const mapKey = `${mapCenter.lat}-${mapCenter.lng}`; // Key for map re-render on center change

    // Determine overall loading/error state before rendering map
    const isReadyToRenderMap = !isLoadingConfig && appConfig && !configError && !isLoadingLocation;

    return (
        <div className="flex flex-col h-screen">
             {/* Header */}
            <header className="bg-indigo-600 text-white p-4 shadow-md z-10 flex justify-between items-center">
                <h1 className="text-2xl font-bold text-center">Event Mapper</h1>
                <div className='text-sm text-right min-w-[200px]'> {/* Added min-width */}
                    {/* Combined Status Display */}
                    {isLoadingConfig && <span>Loading Config...</span>}
                    {configError && <span className="text-red-300 ml-2">{configError}</span>}
                    {!isLoadingConfig && !configError && isLoadingLocation && <span>Locating...</span>}
                    {isReadyToRenderMap && isLoadingEvents && <span>Loading Events...</span>}
                    {/* Display general/event errors if no config/location errors */}
                    {!configError && !isLoadingLocation && error && <span className="text-red-300 ml-2">{error}</span>}
                    {/* Proximity error might show alongside event loading/general errors */}
                     {/* {!configError && proximityError && <span className="text-yellow-300 ml-2">{proximityError}</span>} */}
                     {/* ^ Proximity error moved to bottom bar for better visibility */}
                </div>
            </header>

            {/* Map Area */}
            <div className="flex-grow relative">
                {/* Show Loading/Error states *before* trying to render the map */}
                {isLoadingConfig && ( <div className="absolute inset-0 bg-gray-200 flex items-center justify-center z-20"><p className="text-gray-700 text-xl">Loading Configuration...</p></div> )}
                {!isLoadingConfig && configError && ( <div className="absolute inset-0 bg-red-100 flex items-center justify-center z-20 p-4"><p className="text-red-700 text-xl text-center">{configError}</p></div> )}
                {!isLoadingConfig && !configError && isLoadingLocation && ( <div className="absolute inset-0 bg-gray-200 flex items-center justify-center z-20"><p className="text-gray-700 text-xl">Initializing Map & Location...</p></div> )}

                {/* Render Map only when config is loaded, no error, and location is potentially ready */}
                {isReadyToRenderMap && appConfig?.maps_api_key && (
                    <APIProvider apiKey={appConfig.maps_api_key}>
                        <Map
                            key={mapKey} // Re-renders if center changes significantly
                            defaultCenter={mapCenter}
                            defaultZoom={initialZoom}
                            mapId={appConfig.maps_api_key}
                            gestureHandling="cooperative"
                            onClick={handleMapClick}
                            className="absolute inset-0 w-full h-full"
                            clickableIcons={false} // Prevent clicking Google's default POIs
                        >
                            {/* Permanent Event Markers */}
                            {events.map(event => (
                                <AdvancedMarker key={event.id} position={event.position} title={event.title} onClick={() => handleSelectEvent(event)}>
                                    <Pin background={event.category === 'landmark' ? "#10B981" : "#4F46E5"} glyphColor={"#FFF"} borderColor={"#FFF"}/>
                                </AdvancedMarker>
                            ))}
                            {/* Temporary Marker for New Event */}
                            {temporaryMarkerPosition && (
                                <AdvancedMarker position={temporaryMarkerPosition} title="New Event Location">
                                    <Pin background={"#F59E0B"} glyphColor={"#FFF"} borderColor={"#FFF"} />
                                </AdvancedMarker>
                            )}
                            {/* User Location Marker */}
                             {currentUserPosition && (
                                 <AdvancedMarker position={currentUserPosition} title="Your Location" zIndex={10}>
                                    <UserLocationMarker />
                                </AdvancedMarker>
                             )}
                             {/* Info Window for Selected Event */}
                             {selectedEvent && selectedEvent.position && ( // Added check for position
                                <InfoWindow
                                    position={selectedEvent.position}
                                    onCloseClick={() => setSelectedEvent(null)}
                                    // pixelOffset={[0, -30]} // Optional: adjust position relative to marker pin
                                >
                                     <div className="text-black p-2 max-w-xs">
                                        <h3 className="font-bold text-lg mb-1">{selectedEvent.title}</h3>
                                        <p className="text-sm mb-1"><strong>Category:</strong> {selectedEvent.category}</p>
                                        <p className="text-sm mb-2">{selectedEvent.description}</p>
                                        <p className="text-xs text-gray-600">Starts: {selectedEvent.startTime ? new Date(selectedEvent.startTime).toLocaleString() : 'N/A'}</p>
                                        <p className="text-xs text-gray-600">Ends: {selectedEvent.endTime ? new Date(selectedEvent.endTime).toLocaleString() : 'N/A'}</p>
                                    </div>
                                </InfoWindow>
                             )}
                        </Map>
                    </APIProvider>
                )}
            </div>

            {/* Bottom Tab */}
            <div className="bg-gray-800 text-white p-4 shadow-lg z-10">
                 {/* Display Proximity/General Errors prominently here */}
                 {proximityError && (<div className="mb-2 p-2 bg-yellow-700 text-yellow-100 rounded-md text-sm text-center">{proximityError}</div>)}
                 {error && !proximityError && (<div className="mb-2 p-2 bg-red-800 text-red-100 rounded-md text-sm text-center">{error}</div> )}

                <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
                     {/* Event List */}
                    <div className="w-full md:w-2/3 mb-4 md:mb-0">
                        <h2 className="text-xl font-semibold mb-2">Events {isLoadingEvents ? '(Loading...)' : `(${events.length})`}</h2>
                        <div className="max-h-32 overflow-y-auto pr-2"> {/* Added scrollbar padding */}
                            {!isLoadingConfig && !isLoadingEvents && events.length === 0 && (<p className="text-gray-400">No events found nearby or added yet.</p>)}
                            {(isLoadingConfig || isLoadingEvents) && (<p className="text-gray-400">Loading...</p>)}
                            <ul className="space-y-2">
                                {events.map(event => (
                                    <li key={event.id} className="bg-gray-700 p-2 rounded-md cursor-pointer hover:bg-gray-600 transition-colors" onClick={() => handleSelectEvent(event)}>
                                        <p className="font-semibold">{event.title}</p>
                                        <p className="text-sm text-gray-300 truncate">{event.description}</p>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    </div>
                    {/* Add Button */}
                     <div className="w-full md:w-auto flex justify-center md:justify-end">
                        <button
                            onClick={handleAddEventClick}
                            // Disable button if: no temp marker OR submitting OR config not loaded OR location not ready
                            disabled={!temporaryMarkerPosition || isSubmittingEvent || isLoadingConfig || !appConfig || isLoadingLocation}
                            className={`px-6 py-3 rounded-lg font-semibold shadow-md transition-colors ${
                                temporaryMarkerPosition && !isSubmittingEvent && appConfig && !isLoadingLocation
                                    ? 'bg-green-500 hover:bg-green-600 text-white cursor-pointer'
                                    : 'bg-gray-500 text-gray-300 cursor-not-allowed'
                            }`}
                            title={ // Add helpful title for disabled state
                                !appConfig ? "Waiting for configuration..." :
                                !currentUserPosition ? "Waiting for location..." :
                                !temporaryMarkerPosition ? "Click on the map within the allowed radius to select a location first" :
                                isSubmittingEvent ? "Submitting event..." :
                                "Add New Event at Selected Location"
                            }
                        >
                            Add New Event
                        </button>
                    </div>
                </div>
            </div>

            {/* Modal */}
            <EventFormModal
                isOpen={isModalOpen}
                onClose={handleCloseModal}
                onSubmit={handleEventSubmit}
                position={temporaryMarkerPosition}
                eventDetails={newEventDetails}
                setEventDetails={setNewEventDetails}
                isSubmitting={isSubmittingEvent}
            />
        </div>
    );
}

export default App;